...
//}
//
//int deleteInnerTree(struct BTreeNode* node, int pos) { // TO DO:: 자식이 없을경우는 어떻게 되는거지 ? 
//	int result_deletion = 0; // TO DO:: CHECK
//	int flag = false;
//	if (node->child[pos]->num_key >= node->child[pos + 1]->num_key) { // 왼쪽 자식의 키 갯수가 크거나 같을때
//		result_deletion = findPredecessor(node->child[pos]);
//		flag = deleteValFromNode(result_deletion, node->child[pos]);
//
//	}
//	else {															  // 오른쪽 자식의 키 갯수가 클 때
//		result_deletion = findSuccessor(node->child[pos+1]);
//		flag = deleteValFromNode(result_deletion, node->child[pos+1]);
//	}
//	node->key[pos] = result_deletion;
//	return flag;
//}
//
...
//		for (int i = 0; i < node->child[pos]->num_child; i++) {
//			node->child[pos]->child[i + 1] = node->child[pos]->child[i];
//		}
